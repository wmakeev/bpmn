 ...
 * @constructor
 */
var DebuggerInterface = exports.DebuggerInterface = function DebuggerInterface(url, fileName) {
 ...
 * @returns {boolean}
 */
DebuggerInterface.prototype.isInDebugger = function isInDebugger() {
 ...
 * @param {Function} done
 */
DebuggerInterface.prototype.sendPosition = function sendPosition(flowObject, logger, done) {
  var self = this;
  ...
  try {
    ...
    client.post(self.url.pathname, debuggerMessage, function reqHandler(error, req, res, obj) {
      if (error) {
        logger.error("Error sending position to '" + self.url.href + "'. Error: " + error);
      }

      // client must be closed before calling done() because done might lead to another
      // sendPosition call
      client.close();

      if (done) {
        logger.debug("DebuggerInterface: Sending '" +
          JSON.stringify(debuggerMessage) + "' to '" + self.url.href + "'");
 ...
 * @return {*}
 */
DebuggerInterface.prototype.createRestClient = function createRestClient(url) {
 ...
 * @constructor
 */
exports.createDebuggerInterface = function createDebuggerInterface(node, fileName) {
 ...
 * @return {Boolean}
 */
exports.isDebuggerElement = function isDebuggerElement(node) {
