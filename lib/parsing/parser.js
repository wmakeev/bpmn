...
/**
 * @param {Object} prefix2NamespaceMap Maps prefixes to namespaces.
 *                 ASSUMPTION: prefixes map uniquely to namespaces
 * @param node
 */
function addNamespaces(prefix2NamespaceMap, node) {
  var attributes = node.attributes;
  if (attributes) {
    Object.keys(attributes).forEach(function attributesKeysIter(key) {
 ...
 */
exports.parse = function parse(bpmnXML, errorQueue, mainProcessName, fileName) {
  ...
  parser.onerror = function onerror(err) {
    errorQueue.addError('NOPARSE', {}, err.message);
    parser.resume();
  };
  ...
  parser.onopentag = function onopentag(node) {
    ...
    if (inBPMN2Namespace) {
      if (collaborationDefinition.isCollaborationDefinitionName(localName)) {
        currentCollaborationDefinition = collaborationDefinition
          .createBPMNCollaborationDefinition(node);
      ...
      if (processDefinition.isProcessName(localName)) {
        if (processDefinition.isExecutable(node)) {
          currentProcessDefinition = processDefinition
            .createBPMNProcessDefinition(node, debuggerInterface);
      ...
      if (currentProcessDefinition) {
        if (tasks.isTaskName(localName)) {
          ...
          currentProcessElement = callActivity
            .createBPMNCallActivity(node, fileName, prefix2NamespaceMap,
  ...
  parser.onclosetag = function onclosetag(node) {
