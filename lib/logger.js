...
function getLogLevelString(logLevel) {
  ...
  keys.forEach(function iter(key) {
...
Object.keys(logLevels).forEach(function iter(logLevelName) {
  Logger.prototype[logLevelName] = function logLevel(description, data) {
 ...
 */
Logger.prototype.setProcess = function setProcess(bpmnProcess) {
  if (bpmnProcess) {
 ...
 * @param {number | string} logLevel
 */
Logger.prototype.setLogLevel = function setLogLevel(logLevel) {
 ...
 * @param options
 */
Logger.prototype.addTransport = function addTransport(WinstonTransport, options) {
 ...
 * @param winstonTransport
 */
Logger.prototype.removeTransport = function removeTransport(winstonTransport) {
  this.winstonLogger.remove(winstonTransport);
};
 ...
 * @param {Object=} data
 */
Logger.prototype.log = function log(logLevel, description, data) {
  var processId;
  var processName;
  var dataMessage;
  ...
  if (logLevel >= this.logLevel) {
    ...
    if (this.logAppender) {
      ...
      formattedMessage = '[' + getLogLevelString(logLevel) + '][' + processName + '][' +
        processId + '][' + description + ']' + dataMessage;
      ...
      this.winstonLogger.log(getLogLevelString(logLevel), messageObject, function handler(error) {
        if (error) {
          /* eslint no-console: 0 */
